...
const deleteTask = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      message: `${task.title} deleted and removed from dependent tasks`,
    });
  } catch (error) {
...
const AddingSubtask = async (req, res) => {
  try {
    const { taskId } = req.params;
    const {
      title,
...
const deleteSubtask = async (req, res) => {
  try {
    ...
    const allCompleted =
      ...
      remainingSubtasks.every((s) => s.completed);
...
const updateGanttChart = async (req, res) => {
  const { startingDate, dueDate } = req.body;
  const { id } = req.params;
  if (!startingDate || !dueDate) {
    return res
      .status(400)
      .json({ message: "Both startingDate and dueDate are required" });
  }
...
const updateDependencies = async (req, res) => {
...
const updateSubtasks = async (req, res) => {
  ...
  try {
    ...
    task.activities.unshift({
      ...
      user,
      timestamp: new Date(),
      message: `Updated subtask: ${subtask.title} - ${
        subtask.completed ? "Completed" : "Reopened"
      }`,
    });
    ...
    if (task.status === "COMPLETED" && !subtask.completed) {
      ...
      task.activities.unshift({
        ...
        timestamp: new Date(),
        message:
          "Task moved from COMPLETED to TO DO because a subtask was marked as incomplete",
      });
    } else if (allCompleted && task.status !== "COMPLETED") {
      task.status = "COMPLETED";
      task.stage = "completed";
...
const addComments = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: "Activity added to top successfully",
      task: updatedTask,
    });
  } catch (error) {
...
//add image as assets
const addImageAssets = async (req, res) => {
  const { taskId } = req.params;
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Asset and activity added successfully.", task });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error." });
  }
...
const getAllTransactions = async (req, res) => {
  try {
    const { pid } = req.params;
    const transactions = await Transaction.find({ projectId: pid }).sort({
      createdDate: -1,
    });
...
const approveTheTransactions = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    if (
      approvedById.toString() !== existingTransacton.requestedById.toString()
    ) {
...
const rejectTheTransaction = async (req, res) => {
  try {
    const { id } = req.params;
    const { status, approvedBy, approvedById } = req.body;
...
const getShortedTaskWithAlgo = async (req, res) => {
  try {
...
const getOverViewWithAlgo = async (req, res) => {
  try {
    ...
    if (monthlyData.length >= 2) {
      ...
      const predictedExpenses = weightedLinearRegression(
        monthlyData,
        "expenses"
      );
      ...
      const nextMonth = lastMonthDate.toLocaleString("default", {
        month: "short",
        year: "numeric",
      });
...
const getProjectDetailOverview = async (req, res) => {
  try {
    const { id } = req.params;
...
const editProjectDetails = async (req, res) => {
  try {
    ...
    if (!description || !dueDate) {
      return res
        .status(400)
        .json({ message: "Description and due date are required." });
    }
...
const deleteProject = async (req, res) => {
  try {
    ...
    await Promise.all(
      tasksToDelete.map((task) => Task.findByIdAndDelete(task._id))
    );
