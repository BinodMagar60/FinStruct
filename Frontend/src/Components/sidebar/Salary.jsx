...
import { getRolesAndSalaries, DeleteRolesAndSalaries, getAllUserForSalary, updateUserSalary, addRolesAndSalaries, updateRolesAndSalaries } from "../../api/AdminApi";
...
export default function Salary() {
  ...
  const handleSaveRole = async (updatedRole) => {
    try{
      const response = await updateRolesAndSalaries(`admin/user/roles-salaries/${updatedRole._id}`, updatedRole)
      // console.log(response)
      setChanging(prev=> !prev)
    }
    catch(err){
  ...
  const handleAddNewRole = async (newRole) => {
    try {
      ...
      const response = await addRolesAndSalaries("admin/user/roles-salaries", newRole);
      setChanging(prev => !prev)
      setShowAddRoleModal(false);
    } catch (err) {
      console.log("Error adding new role: ", err);
      toast.error(err.message, { theme: 'light', autoClose: 1500 });
    }
  };
  ...
  const handleDeleteRole = async (id) => {
    try {
      ...
      // Remove the role from the local state after successful deletion
