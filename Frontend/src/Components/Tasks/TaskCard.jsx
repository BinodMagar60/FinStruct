...
const TaskCard = ({ task, columnId }) => {
  ...
  // Only allow dragging if the task is not in the completed column
  ...
  return (
    <div
      ref={drag}
      className={`bg-white border border-gray-200 rounded-lg mb-2 ${isDragging ? "opacity-50" : ""} ${
        isCompleted ? "bg-green-50" : ""
      }`}
      style={{ opacity: isDragging ? 0.5 : 1, cursor: isCompleted ? "default" : "grab" }}
    >
      <div className="p-3">
        {task.priority === "high" && <div className="text-red-500 text-xs mb-1">High Priority</div>}
        {task.priority === "normal" && <div className="text-yellow-500 text-xs mb-1">Normal Priority</div>}
        {task.priority === "low" && <div className="text-green-500 text-xs mb-1">Low Priority</div>}

        <div className="flex justify-between items-start">
          <div className="flex items-center gap-2">
            <div className={`w-4 h-4 rounded-full ${isCompleted ? "bg-green-500" : "bg-blue-500"}`}></div>
        ...
        {task.subtasks && task.subtasks.length > 0 && (
          <div className="mt-2">
            {task.subtasks.map((subtask) => (
              <div key={subtask.id} className="flex items-center gap-2 mt-1 py-1 border-t border-gray-100">
                <input
                  type="checkbox"
                ...
                />
                <label htmlFor={subtask.id} onClick={(e) => e.stopPropagation()}>
                  <span
      ...
      <div className="border-t border-gray-200 px-3 py-2">
        {showSubtaskForm ? (
          <form onSubmit={submitSubtask} className="flex flex-col gap-2" onClick={(e) => e.stopPropagation()}>
            <input
              ref={subtaskInputRef}
              type="text"
              value={subtaskTitle}
